...
$api->group(['prefix' => 'users', 'namespace' => 'User'], function (Router $api) {
    ...
    $api->group(['middleware' => ['auth']], function (Router $api) {
        ...
        $api->put('/profile/update', ['as' => 'user.profile.update', 'uses' => 'UserController@profileUpdate']);
        ...
        $api->group(['prefix' => 'orders'], function (Router $api) {
            ...
            $api->post('/{order}/comment', ['as' => 'user.order.comment', 'uses' => 'OrderController@commentOrder']);
            $api->get('/{order}/offers', ['as' => 'user.order.offers', 'uses' => 'OrderController@offerOrders']);
            $api->post('/{order}/addition/fee', ['as' => 'user.order.addition.fee', 'uses' => 'OrderController@addAdditionOrder']);
        ...
        $api->group(['prefix' => 'complaints'], function (Router $api) {
            ...
            $api->post('/', ['as' => 'user.order.complaint', 'uses' => 'ComplaintController@store']);// 发起投诉
        });
        /**
         * 商品
         */
        ...
        $api->group(['prefix' => 'refunds'], function (Router $api) {
            ...
            $api->post('/apply', ['as' => 'user.order.applyRefund', 'uses' => 'RefundOrderController@initRefund']);// 发起退款
